#!/usr/bin/env bash
# Play / evaluate the Push-task checkpoint from this workspace's logs/.
set -euo pipefail
...
CKPT_DIR="${WORKSPACE_DIR}/logs/rl_games/widowx_pcb_push/nn"
LEGACY_CKPT_DIR="${WORKSPACE_DIR}/logs/rl_games/widowx_pcb_straddle/nn"
DEFAULT_CKPT_NAME="widowx_pcb_push.pth"
LEGACY_CKPT_NAME="widowx_pcb_straddle.pth"

if [[ ! -d "${CKPT_DIR}" && -d "${LEGACY_CKPT_DIR}" ]]; then
  echo "[WARN] Using legacy log dir: ${LEGACY_CKPT_DIR} (migrate to widowx_pcb_push)" >&2
...
resolve_checkpoint() {
  ...
  if [[ -z "${ckpt}" || ! -f "${ckpt}" ]]; then
    ...
    echo "        Train first (bash scripts/train_push.sh), or pass --checkpoint <path>.pth" >&2
    exit 1
  fi
  echo "${ckpt}"
}
