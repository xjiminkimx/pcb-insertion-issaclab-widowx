#!/usr/bin/env python3
"""One-off: compute the magazine link bbox/centre WITHOUT regenerating the USD.

Reuses the parsing/FK helpers from convert_to_usd.py so we can derive the true
insert target (magazine geometric centre) while training is using the USD file.
"""
...
fk = c.compute_fk(joints, root_link="root")
...
for link_name in sorted(c.MAGAZINE_LINKS):
    ...
    print(f"[ok] {link_name}: {len(points)} pts")
...
print("\nMagazine bbox in USD root frame:")
...
# _MAG_POS = (tx, ty, tz); world_x = root_Y + tx, world_y = -root_X + ty, world_z = root_Z + tz
tx, ty, tz = (-0.380, 0.094, 0.350)
wx = cy + tx
wy = -cx + ty
wz = cz + tz
print(f"\nMagazine centre WORLD (with _MAG_POS=({tx},{ty},{tz})):")
print(f"  ({wx:.4f}, {wy:.4f}, {wz:.4f})")
print(f"\nSlot-mouth (assembly root X-min mapping) world Y reference = 0.490")
